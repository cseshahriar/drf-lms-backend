...
LANGUAGE = (
    ("Bengali", "Bengali"),        # 230 million (Your original "Bangla")
    ("English", "English"),        # 1.5 billion speakers (Global)
    ("Mandarin", "Mandarin"),      # 1.1 billion (Most native speakers)
    ("Hindi", "Hindi"),            # 600 million (Including Urdu)
    ("Spanish", "Spanish"),        # 550 million
    ("French", "French"),          # 300 million (Official UN language)
    ("Arabic", "Arabic"),          # 300 million (All varieties)
    ("Portuguese", "Portuguese"),  # 230 million
    ("Russian", "Russian"),        # 150 million
    ("German", "German"),          # 130 million
    ("Japanese", "Japanese"),      # 125 million
)
...
class Course(models.Model):
    ...
    level = models.CharField(
        choices=LEVEL, default="Beginner", max_length=100, blank=True,
        null=True
    )
    ...
    def average_rating(self):
        average_rating = Review.objects.filter(
            course=self, active=True).aggregate(
