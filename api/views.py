...
class PasswordResetEmailVerifyAPIView(generics.RetrieveAPIView):
    ...
    def get_object(self):
        email = self.kwargs['email']  # api/v1/password-email-verify/desphixs@gmail.com/  # noqa
...
class ChangePasswordAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if user is not None:
            ...
            return Response({
                "message": "User does not exists", "icon": "error"})
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            checkout_session = stripe.checkout.Session.create(
                ...
                mode='payment',
                success_url=settings.FRONTEND_SITE_URL + '/payment-success/' + order.oid + '?session_id={CHECKOUT_SESSION_ID}',  # noqa
            ...
            return Response(
                {
                    "message": f"Something went wrong when trying to make payment. Error: {str(e)}"  # noqa
                }
            )
...
class PaymentSuccessAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if paypal_order_id != "null":
            paypal_api_url = f"https://api-m.sandbox.paypal.com/v2/checkout/orders/{paypal_order_id}"  # noqa
            headers = {
                'Content-Type': 'application/json',
                'Authorization': f"Bearer {get_access_token(PAYPAL_CLIENT_ID, PAYPAL_SECRET_ID)}"  # noqa
            }
...
class StudentNoteCreateAPIView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return api_models.Note.objects.filter(
            user=user, course=enrolled.course
        )
...
class StudentWishListListCreateAPIView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if wishlist:
            wishlist.delete()
            return Response({
                "message": "Wishlist Deleted"}, status=status.HTTP_200_OK)
        else:
...
class TeacherReviewDetailAPIView(generics.RetrieveUpdateAPIView):
    ...
    def get_object(self):
        ...
        return api_models.Review.objects.get(
            course__teacher=teacher, id=review_id)
...
class CourseUpdateAPIView(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        elif 'image' in request.data and str(request.data['image']) == "No File":  # noqa
            course.image = None

        if 'file' in request.data and not str(request.data['file']).startswith("http://"):  # noqa
            course.file = request.data['file']

        if 'category' in request.data['category'] and request.data['category'] != 'NaN' and request.data['category'] != "undefined":  # noqa
            ...
            if key.startswith("variants") and '[variant_title]' in key:
                ...
                if existing_variant:
                    ...
                    for item_data in item_data_list[1:]:
                        ...
                        preview = bool(strtobool(str(preview_value))) if preview_value is not None else False  # noqa

                        variant_item = api_models.VariantItem.objects.filter(
                            variant_item_id=item_data.get("variant_item_id")
                        ).first()

                        if not str(item_data.get("file")).startswith("http://"):  # noqa
                            ...
                            if variant_item:
                                ...
                                variant_item = api_models.VariantItem.objects.create(  # noqa
...
class CourseVariantDeleteAPIView(generics.DestroyAPIView):
    ...
    def get_object(self):
        ...
        teacher = api_models.Teacher.objects.get(id=teacher_id)
        course = api_models.Course.objects.get(  # noqa
...
class FileUploadAPIView(APIView):
    ...
    @swagger_auto_schema(
        ...
        request_body=api_serializer.FileUploadSerializer,  # Use serializer
        responses={
            200: openapi.Response(
                'File uploaded successfully', openapi.Schema(
        ...
        if serializer.is_valid():
            ...
            if file.name.endswith(('.mp4', '.avi', '.mov', '.mkv')):
                ...
                file_full_path = os.path.join(
                    default_storage.location, file_path)
